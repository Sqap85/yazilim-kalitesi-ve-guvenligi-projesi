...
def add_to_cart():
    ...
    if quantity is None or quantity <= 0:
        return jsonify({'success': False, 'message': 'Geçerli bir adet girin'}), 400
    ...
    existing_quantity = cart.get(pid_str, {}).get('quantity', 0)
...
def update_cart():
    ...
    if product_id is None or quantity is None:
        return jsonify({'success': False, 'message': 'Geçersiz sepet isteği'}), 400

    product_id = str(product_id)
    cart = get_cart()

    if product_id in cart:
        product = get_product_by_id(cart[product_id]['product_id'])
        if quantity > 0 and product and quantity > product['stock']:
            return jsonify({'success': False, 'message': 'Yetersiz stok'}), 400
        if quantity <= 0:
            del cart[product_id]
        else:
...
@app.route('/health')
def health():
    return jsonify({'status': 'healthy', 'service': 'techstore', 'version': APP_VERSION})
