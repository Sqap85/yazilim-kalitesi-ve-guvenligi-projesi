...
def test_add_to_cart_rejects_zero_quantity(client):
    """Sıfır veya negatif adet kabul edilmemeli."""
    response = client.post('/api/cart/add',
...
def test_add_to_cart_respects_cumulative_stock(client):
    """Aynı ürün tekrar eklendiğinde toplam adet stok sınırını aşmamalı."""
    client.post('/api/cart/add',
        data=json.dumps({'product_id': 7, 'quantity': 4}),
        content_type='application/json'
    )
    response = client.post('/api/cart/add',
        data=json.dumps({'product_id': 7, 'quantity': 2}),
        content_type='application/json'
    )
    assert response.status_code == 400
    data = json.loads(response.data)
...
def test_update_cart_rejects_quantity_above_stock(client):
    """Sepet güncelleme stok üstüne çıkmamalı."""
    client.post('/api/cart/add',
        data=json.dumps({'product_id': 4, 'quantity': 1}),
        content_type='application/json'
    )
    response = client.post('/api/cart/update',
        data=json.dumps({'product_id': 4, 'quantity': 999}),
        content_type='application/json'
    )
    assert response.status_code == 400
    data = json.loads(response.data)
...
def test_checkout_clears_cart(client):
    """Başarılı sipariş sonrası sepet temizlenmeli."""
    client.post('/api/cart/add',
    ...
    response = client.get('/cart')
    assert 'Sepetiniz boş' in response.data.decode('utf-8')
